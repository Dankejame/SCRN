...
def sample_neg(net, test_ratio=0.1, train_pos=None, test_pos=None, max_train_num=None,
    ...
    if max_train_num is not None and train_pos is not None:
        ...
        perm_test = np.random.permutation(len(test_pos[0]))[:max_train_num]
        train_pos = (train_pos[0][perm], train_pos[1][perm])
        test_pos = (test_pos[0][perm_test], test_pos[1][perm_test])
    # sample negative links for train/test
    print('test_num = {}'.format(len(test_pos[0])))
    ...
    if not all_unknown_as_negative:
        ...
        test_neg = test_neg[:len(train_neg[0])][:len(train_neg[0])]
...
def subgraph_extraction_labeling(ind, A, h=1, max_nodes_per_hop=None,
    ...
    dist = 0
    #print('subgraph_extraction_labeling')
    ...
    # construct nx graph
    g = nx.from_scipy_sparse_matrix(subgraph) #压缩
